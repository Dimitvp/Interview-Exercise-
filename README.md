# Interview-Exercise-
My exercises on job interview tests.
Some resources I've used:
 ## C#:
 
   - [TestDome](https://www.testdome.com/tests/c-sharp-online-test/18)
   - [LeetCode](https://leetcode.com/discuss/interview-question/object-oriented-design/?orderBy=most_votes)
   
 ## SQL:
 
  - [TestDome](https://www.testdome.com/tests/sql-online-test/12)
  - [SQL zoo](http://sqlzoo.net/wiki/SQL_Tutorial)
  - [software testing help](https://www.softwaretestinghelp.com/50-popular-sql-interview-questions-for-testers/)
